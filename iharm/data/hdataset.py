...
class HDataset(BaseHDataset):
    ...
    def get_sample(self, index):
        ...
        composite_image_lab = cv2.cvtColor(composite_image, cv2.COLOR_RGB2Lab)

        real_image = cv2.imread(real_image_path)
        real_image = cv2.cvtColor(real_image, cv2.COLOR_BGR2RGB)

        real_image_lab = cv2.cvtColor(real_image, cv2.COLOR_RGB2Lab)
        ...
        return {
            ...
            'image_id': index,
            'comp_img_lab' :composite_image_lab,
            'real_img_lab' :real_image_lab,
            'comp_img_name':composite_image_name,
        }
