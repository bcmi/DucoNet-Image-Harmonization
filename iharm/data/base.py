...
class BaseHDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        comp_img_lab = sample['comp_img_lab']
        real_img_lab = sample['real_img_lab']

        comp_image_lab = transform(comp_img_lab)
        real_image_lab = transform(real_img_lab)

        output = {
            'images': image,
            'masks': obj_mask[np.newaxis, ...].astype(np.float32),
            'target_images': target_image,
            'comp_img_lab' : comp_image_lab,
            'real_img_lab' : real_image_lab,
        }
    ...
    def check_augmented_sample(self, sample, aug_output):
        ...
        return aug_output['object_mask'].sum() > 10.0
