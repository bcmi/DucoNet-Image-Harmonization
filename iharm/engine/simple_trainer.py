...
class SimpleHTrainer(object):
    ...
    def training(self, epoch):
        ...
        logger.info(f'Epoch {epoch}, train loss: {train_loss / num_batches:.6f}')
    ...
    def validation(self, epoch):
        ...
        logger.info(f'Epoch {epoch}, validation loss: {val_loss / num_batches:.6f}')
        for metric in self.val_metrics:
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            images_lab = batch_data['comp_img_lab']

            output = self.net(images, masks,image_lab = images_lab)
